...
int no_automata = 1;
 public int exprIndex =RegExpHolder .parseAndStore ("[p] makeGoldUser.( openSession.deposit + openSession.withdraw) + makeGoldUser.(closeSession.deposit + closeSession.withdraw) + makeSilverUser;");
...
if (0==1){}
		...
		if (1==0){}
		else if ((_occurredEvent(_event,14/*evt_LarvaReset*/))){
...
reh =new RegExpHolder (exprIndex );
		...
		if (1==0){}
		else if ((_occurredEvent(_event,16/*evt_LarvaStop*/))){
		reh .stop ();
...
currentState =-1 ;
		...
		else if ((_occurredEvent(_event,4/*evt_deposit*/)) && (reh .toDerivative (Sym_deposit ).isEmpty ())){
		reh .stop ();
...
currentState =-1 ;
		...
		else if ((_occurredEvent(_event,6/*evt_withdraw*/)) && (reh .toDerivative (Sym_withdraw ).isEmpty ())){
		reh .stop ();
...
currentState =-1 ;
		...
		else if ((_occurredEvent(_event,8/*evt_closeSession*/)) && (reh .toDerivative (Sym_closeSession ).isEmpty ())){
		reh .stop ();
...
currentState =-1 ;
		...
		else if ((_occurredEvent(_event,10/*evt_makeSilverUser*/)) && (reh .toDerivative (Sym_makeSilverUser ).isEmpty ())){
		reh .stop ();
