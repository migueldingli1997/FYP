...
int no_automata = 1;
 public int exprIndex =RegExpHolder .parseAndStore ("(makeGoldUser.(openSession .deposit + openSession.withdraw))[2,4] + (makeGoldUser.(closeSession.deposit + closeSession.withdraw))[5,7];");
...
if (0==1){}
		...
		if (1==0){}
		else if ((_occurredEvent(_event,12/*evt_LarvaReset*/))){
...
reh =new RegExpHolder (exprIndex );
		...
		if (1==0){}
		else if ((_occurredEvent(_event,14/*evt_LarvaStop*/))){
		reh .stop ();
		...
		if (1==0){}
		else if ((_occurredEvent(_event,10/*timeout_isE*/)) && (reh .getId ()==id )){
		reh .stop ();
