...
int no_automata = 1;
 public int exprIndex =RegExpHolder .parseAndStore ("[n] ADMIN_initialise.ADMIN_createUser; {ADMIN_reconcile}");
...
currentState =-1 ;
		...
		else if ((_occurredEvent(_event,2/*evt_ADMIN_createUser*/)) && (reh .toDerivative (Sym_ADMIN_createUser ).hasEmptyString ())){
		reh .stop ();
...
currentState =-1 ;
		...
		else if ((_occurredEvent(_event,4/*evt_ADMIN_reconcile*/)) && (reh .toDerivative (Sym_ADMIN_reconcile ).hasEmptyString ())){
		reh .stop ();
...
currentState =1 ;
		...
		else if ((_occurredEvent(_event,2/*evt_ADMIN_createUser*/)) && (reh .getRegExp ().isEmpty ())){
		reh .stop ();
...
currentState =1 ;
		...
		else if ((_occurredEvent(_event,4/*evt_ADMIN_reconcile*/)) && (reh .getRegExp ().isEmpty ())){
		reh .stop ();
...
case 1: if (_mode == 0) return "bad"; else return "!!!SYSTEM REACHED BAD STATE!!! bad "+new _BadStateExceptionscript_1().toString()+" ";
