...
parent.channel_stateSkip .send (new StateSkip (new AutomatonInstance (false ),"start_to_S2",new Clock []{}));
...
if (0==1){}
		...
		_state_id_property = 9;//moving to state good
parent.channel_TAsToMain .send (new Outcome ("good"));
_killThis ();
		...
		if (1==0){}
		else if ((_occurredEvent(_event,13/*evt_channelStateSkip*/)) && (ss .getDestination ().equals ("start_to_S0"))){
		
		_state_id_property = 4;//moving to state S0
		_goto_property(_info);
		}
		else if ((_occurredEvent(_event,13/*evt_channelStateSkip*/)) && (ss .getDestination ().equals ("start_to_S2"))){
...
_killThis ();
		...
		if (1==0){}
		else if ((_occurredEvent(_event,2/*evt_ADMIN_reconcile*/))){
...
switch(_state_id){
case 7: if (_mode == 0) return "S3"; else return "S3";
...
case 9: if (_mode == 0) return "good"; else return "good";
