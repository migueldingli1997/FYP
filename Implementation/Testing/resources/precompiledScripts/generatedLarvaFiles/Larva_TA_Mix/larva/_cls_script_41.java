...
parent.channel_stateSkip .send (new StateSkip (new AutomatonInstance (false ),"start_to_S1",new Clock []{clock_0 }));
}}
}
...
if (clock_0 .current ()<0.5 ){parent.instanceCount ++;
parent.channel_stateSkip .send (new StateSkip (new AutomatonInstance (false ),"S2_to_S0",new Clock []{clock_0 }));
}if (clock_0 .current ()<0.5 ){parent.instanceCount ++;
parent.channel_stateSkip .send (new StateSkip (new AutomatonInstance (true ),"S2_to_start",new Clock []{null }));
}parent.channel_TAsToMain .send (new Outcome ("bad"));
_killThis ();

		_goto_property(_info);
		}
		else if ((_occurredEvent(_event,14/*evt_channelStateSkip*/)) && (ss .getDestination ().equals ("start_to_S1"))){
		setClocks (ss ,clock_0 );

		_state_id_property = 4;//moving to state S1
		_goto_property(_info);
		}
		else if ((_occurredEvent(_event,14/*evt_channelStateSkip*/)) && (ss .getDestination ().equals ("S2_to_S0"))){
		setClocks (ss ,clock_0 );

		_state_id_property = 3;//moving to state S0
		_goto_property(_info);
		}
		else if ((_occurredEvent(_event,14/*evt_channelStateSkip*/)) && (ss .getDestination ().equals ("S2_to_start"))){
		setClocks (ss ,clock_0 );
