...
public _cls_script_12( UserAccount _ua,UserInfo _ui) {
parent = _cls_script_11._get_cls_script_11_inst( _ui);
this._ua = _ua;
}
...
public static _cls_script_12 _get_cls_script_12_inst( UserAccount _ua,UserInfo _ui) { synchronized(_cls_script_12_instances){
_cls_script_12 _inst = new _cls_script_12( _ua,_ui);
...
public boolean equals(Object o) {
 ...
 && (_ua == null || _ua.equals(((_cls_script_12)o)._ua))
...
if (0==1){}
		...
		if (1==0){}
		else if ((_occurredEvent(_event,4/*evt_LarvaReset*/))){
...
reh =new RegExpHolder (parent.parent.exprIndex );
		...
		if (1==0){}
		else if ((_occurredEvent(_event,6/*evt_LarvaStop*/))){
		reh .stop ();
		...
		if (1==0){}
		else if ((_occurredEvent(_event,2/*timeout_isE*/)) && (reh .getId ()==parent.parent.id )){
		reh .stop ();
...
currentState =-1 ;
		...
		else if ((_occurredEvent(_event,10/*evt_deposit_ui_ua*/)) && (reh .toDerivative (parent.parent.Sym_deposit_ui_ua ).isEmpty ())){
		reh .stop ();
...
currentState =-1 ;
		...
		else if ((_occurredEvent(_event,0/*evt_ADMIN_initialise*/)) && (reh .toDerivative (parent.parent.Sym_ADMIN_initialise ).isEmpty ())){
		reh .stop ();
...
currentState =-1 ;
		...
		else if ((_occurredEvent(_event,8/*evt_openSession_ui*/)) && (reh .toDerivative (parent.parent.Sym_openSession_ui ).isEmpty ())){
		reh .stop ();
...
currentState =-1 ;
		...
		else if ((_occurredEvent(_event,12/*evt_withdraw_ui_ua*/)) && (reh .toDerivative (parent.parent.Sym_withdraw_ui_ua ).isEmpty ())){
		reh .stop ();
