...
public class _cls_script_70 implements _callable{

public static PrintWriter pw; 
public static _cls_script_70 root;

public static LinkedHashMap<_cls_script_70,_cls_script_70> _cls_script_70_instances = new LinkedHashMap<_cls_script_70,_cls_script_70>();
static{
try{
pw = new PrintWriter("larvaRuntimeOutput//output_script_7.txt");

root = new _cls_script_70();
_cls_script_70_instances.put(root, root);
  root.initialisation();
}catch(Exception ex)
{ex.printStackTrace();}
}

_cls_script_70 parent; //to remain null - this class does not have a parent!
public static LarvaController __1;
public static LarvaController __0;
public static RegExpHolder r;
public static int id;
int no_automata = 1;
 public int exprIndex =RegExpHolder .parseAndStore ("[n] ~ADMIN_initialise; {ADMIN_reconcile}");
...
public static _cls_script_70 _get_cls_script_70_inst() { synchronized(_cls_script_70_instances){
 return root;
}
}

public boolean equals(Object o) {
 if ((o instanceof _cls_script_70))
{return true;}
else
{return false;}
}
...
synchronized(_cls_script_70_instances){
...
_cls_script_70[] a = new _cls_script_70[1];
synchronized(_cls_script_70_instances){
a = _cls_script_70_instances.keySet().toArray(a);}
for (_cls_script_70 _inst : a)
...
if (--no_automata == 0){
synchronized(_cls_script_70_instances){
_cls_script_70_instances.remove(this);}
}
...
reh =new RegExpHolder (exprIndex );
		...
		if (1==0){}
		else if ((_occurredEvent(_event,6/*timeout_hasE*/)) && (reh .getId ()==id )){
		reh .stop ();
...
_cls_script_70.pw.println("[property]MOVED ON METHODCALL: "+ _info +" TO STATE::> " + state_format);
_cls_script_70.pw.flush();
}
}
...
case 1: if (_mode == 0) return "bad"; else return "!!!SYSTEM REACHED BAD STATE!!! bad "+new _BadStateExceptionscript_7().toString()+" ";
