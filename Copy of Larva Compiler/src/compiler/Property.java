...
public class Property extends Compiler {
	...
	public void appendJava(StringBuilder sb, Global g, Global root)throws ParseException
	{
		...
		if (!Compiler.light)
		{ 
			sb.append("\r\n String state_format = _string_"+name+"(_state_id_"+name+", 1);");
			if (!Compiler.verbose)
			{
				sb.append("\r\n if (state_format.startsWith(\"!!!SYSTEM REACHED BAD STATE!!!\")) {");
			}
			if (!Compiler.console){
				sb.append("\r\n_cls_"+root.name+root.id+".pw.println(\"[" + name + "]MOVED ON METHODCALL: \"+ _info +\" TO STATE::> \" + state_format);");
				sb.append("\r\n_cls_"+root.name+root.id+".pw.flush();");
			}
			else if (Compiler.console){
				sb.append("\r\n   System.out.println(\"[" + name + "]MOVED ON METHODCALL: \"+ _info +\" TO STATE::> \" + state_format);");
			}
			if (!Compiler.verbose)
			{
				sb.append("\r\n}");
